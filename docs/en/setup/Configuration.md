...
| service_instance | SW_AGENT_SERVICE_INSTANCE | <class 'str'> | fbc57c289f9c11edafd4b2d6bc9cb174 | The name of the Python service instance |
...
| namespace | SW_AGENT_NAMESPACE | <class 'str'> |  | The agent namespace of the Python service |
| collector_address | SW_AGENT_COLLECTOR_ADDRESS | <class 'str'> | 127.0.0.1:11800 | The backend OAP server address |
| kafka_bootstrap_servers | SW_AGENT_KAFKA_BOOTSTRAP_SERVERS | <class 'str'> | localhost:9092 | A list of host/port pairs to use for establishing the initial connection to the Kafka cluster. It is in the form host1:port1,host2:port2,... |
| kafka_topic_management | SW_AGENT_KAFKA_TOPIC_MANAGEMENT | <class 'str'> | skywalking-managements | Specifying Kafka topic name for service instance reporting and registering. |
| kafka_topic_segment | SW_AGENT_KAFKA_TOPIC_SEGMENT | <class 'str'> | skywalking-segments | Specifying Kafka topic name for Tracing data. |
| kafka_topic_log | SW_AGENT_KAFKA_TOPIC_LOG | <class 'str'> | skywalking-logs | Specifying Kafka topic name for Log data. |
| kafka_topic_meter | SW_AGENT_KAFKA_TOPIC_METER | <class 'str'> | skywalking-meters | Specifying Kafka topic name for Meter data. |
| force_tls | SW_AGENT_FORCE_TLS | <class 'bool'> | False | Use TLS for communication with server (no cert required) |
...
| logging_level | SW_AGENT_LOGGING_LEVEL | <class 'str'> | INFO | The logging level, could be one of `CRITICAL`, `FATAL`, `ERROR`, `WARN`(`WARNING`), `INFO`, `DEBUG` |
| disable_plugins | SW_AGENT_DISABLE_PLUGINS | <class 'list'> | [''] | The name patterns in CSV pattern, plugins whose name matches one of the pattern won't be installed |
| max_buffer_size | SW_AGENT_MAX_BUFFER_SIZE | <class 'int'> | 10000 | The maximum queue backlog size for sending the segment data to backend, segments beyond this are silently dropped |
...
| Configuration | Environment Variable | Type | Default Value | Description |
| :------------ | :------------ | :------------ | :------------ | :------------ |
| profiler_active | SW_AGENT_PROFILER_ACTIVE | <class 'bool'> | True | If `True`, Python agent will enable profile when user create a new profile task. Otherwise disable profile. |
...
| log_reporter_safe_mode | SW_AGENT_LOG_REPORTER_SAFE_MODE | <class 'bool'> | False | If `True`, Python agent will filter out HTTP basic auth information from log records. Otherwise, it disables the feature due to potential performance impact brought by regular expression |
